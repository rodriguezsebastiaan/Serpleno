...
import { supabase, envConfig } from '../lib/supabaseClient.js';
...
async function loadSlides() {
  try {
    const { data, error } = await supabase.storage
      .from(envConfig.SUPABASE_STORAGE_SLIDES_BUCKET)
      .list('', { limit: 12, sortBy: { column: 'name', order: 'asc' } });
    if (error) throw error;
    const files = (data || []).filter((f) => /\.(png|jpe?g|gif)$/i.test(f.name));
    if (!files.length) throw new Error('no files');
    return files.map((f) => ({ name: f.name, src: supabase.storage.from(envConfig.SUPABASE_STORAGE_SLIDES_BUCKET).getPublicUrl(f.name).data.publicUrl }));
  } catch (err) {
